...
from tools_app import trouver_id
...
def page_details():
    ...
    if not selected_info.empty:
        ...
        with col2:
            ...
            st.markdown(f"<h2 class='custom-title' style='color: white;'>{selected_info['title']} ({start_year})</h2>", unsafe_allow_html=True)
            ...
            director_profile_path = selected_info.get('director_profile_path', '')
    ...
    similar_movies = selected_info.get('similar_movies', [])
    if similar_movies:
        for movie in similar_movies:
            st.markdown(f"<p style='color: white;'>{movie}</p>", unsafe_allow_html=True)  # Texte des films similaires en blanc
    else:
        st.write("<p style='color: white;'>Aucun film similaire disponible.</p>", unsafe_allow_html=True)  # Texte d'absence en blanc
...
def page_personnage():
    ...
    if selected_person in data['director'].values:
        ...
        filmography = person_info['director_knownForTitles'].split(',')
        ...
        for col in actor_columns:
            if selected_person in data[col].values:
                ...
                filmography = person_info[f'{col}_knownForTitles'].split(',')
                person_type = f"actor_{actor_columns.index(col) + 1}"
                break
    
    if person_info is None:
        st.write("Informations sur la personne non trouvées.")
        return

    # Enregistrer la personne sélectionnée dans la session pour y revenir plus tard
    ...
    with col1:
        ...
        st.markdown(
            f'<img src="{person_image_url}" width="300" style="border-radius: 20px; border: 2px solid #555555; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.3);" />',
            unsafe_allow_html=True
        )
